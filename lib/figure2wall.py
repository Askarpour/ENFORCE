...
def figure2wall(imageName):
	...
	gray=255-gray
	#contours,hierarchy = cv2.findContours(gray,cv2.RETR_LIST ,cv2.CHAIN_APPROX_NONE ) #vecchio
	...
	i=0
	for coord in list:
			coordinateMapx[coord[0][0]]=set()
			coordinateMapy[coord[0][1]]=set()
#	        x.append(coord[0][0])
#	        y.append(coord[0][1])
	for coord in list:
			coordinateMapx[coord[0][0]].add(coord[0][1])
			coordinateMapy[coord[0][1]].add(coord[0][0])

	return (coordinateMapx, coordinateMapy)
#	return (x,y)
